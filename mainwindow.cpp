...
/*
    QBoxLayout *biglay = new QBoxLayout(QBoxLayout::LeftToRight,  this);//main layout
    QBoxLayout *controllButtonsLay = new QBoxLayout(QBoxLayout::LeftToRight);//layout with 2 big buttons
    QBoxLayout *rightLay = new QBoxLayout(QBoxLayout::TopToBottom);//layout with 3 buttons vertical orientation
    QGridLayout *settingsLay = new QGridLayout;//grid layout general for settings
    QBoxLayout *logLayout = new QBoxLayout(QBoxLayout::LeftToRight);//bottom layout for log

    biglay->addLayout(controllButtonsLay, 4);
    biglay->addLayout(rightLay, 1);
    setLayout(biglay);

    biglay->setSpacing(PADDING);
    biglay->setMargin(0);

    controllButtonsLay->setSpacing(PADDING);
    controllButtonsLay->setMargin(0);

    rightLay->setSpacing(PADDING/2);
...
*/
    ...
    logButton->setGeometry(MAXW-SMALLBUTTONSIZE*2-PADDING, PADDING*3 +SMALLBUTTONSIZE*2+5, SMALLBUTTONSIZE*2, SMALLBUTTONSIZE*2-5);
    logButton->setText("Show log");
    logButton->setToolTip("Show log");

    optionsButton = new QPushButton(this);
    optionsButton->setGeometry(MAXW-SMALLBUTTONSIZE*2-PADDING, SMALLBUTTONSIZE*2, SMALLBUTTONSIZE*2, SMALLBUTTONSIZE*2-5);
    optionsButton->setText("Settings");
    optionsButton->setToolTip("Manage Settings");
    ...
    zipLE->setGeometry(150, PADDING/2, MAXW/2+PADDING*2, PADDING);
    ...
    checkConfigurationCB = new QCheckBox("Check configuration", boxWidget);
    checkConfigurationCB->move(140, 60);
    checkConfigurationCB->setCheckState(Qt::CheckState(checkConfigurationState));

    convertConfCB = new QCheckBox("Convert configuration", boxWidget);
    convertConfCB->move(140, 90);
    convertConfCB->setCheckState(Qt::CheckState(convertConfigurationState));


    /*Setup Position and geometry*/
    /*
    controllButtonsLay->addWidget(openDirButton ,1);
    controllButtonsLay->addWidget(openFileButton, 1);

    rightLay->addWidget(closeButton, 1);
    rightLay->addWidget(logButton, 1);
    ...
    */
    ...
    connect(checkConfigurationCB, SIGNAL(stateChanged(int)), this, SLOT(onCheckConfStateChanged(int)));
    connect(convertConfCB, SIGNAL(stateChanged(int)), this, SLOT(onConvertConfStateChanged(int)));
}
...
void MainWindow::initSettings()
{
    ...
    if (file.open(QIODevice::ReadOnly)){
        ...
        convertConfigurationState = true;
    }
    file.close();
}
...
void MainWindow::onCheckConfStateChanged(int state)
{
    checkConfigurationState = state;
    setButtonsText();
}

void MainWindow::onConvertConfStateChanged(int state)
{
    convertConfigurationState = state;
    setButtonsText();
}
...
void MainWindow::setButtonsText()
{
    ...
    if (convertConfigurationState!=0)
        buttonText += "Convert";
    (convertConfigurationState&checkConfigurationState) ? buttonText +=" and " :buttonText +=" " ;
    if (checkConfigurationState!=0)
        buttonText += "Check";
    buttonText += "\nConfiguration ";
